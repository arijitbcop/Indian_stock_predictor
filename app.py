...
def fetch_stock_data(symbol):
    ...
    # Try different possible formats for Indian stocks
    suffixes = [".NS", ".BO", ""]  # NS for NSE, BO for BSE
    
    for suffix in suffixes:
        try:
            full_symbol = symbol + suffix
            print(f"Trying symbol: {full_symbol}")
            stock = yf.Ticker(full_symbol)
            data = stock.history(period="1y")
            print(f"Data retrieved for {full_symbol}, shape: {data.shape}")
            ...
            print(f"Error fetching {symbol + suffix}: {str(e)}")
            continue
    
    # If we reach here, try searching for the symbol
    try:
        symbol_search = symbol.upper()
        if "." not in symbol_search:  # Only search if no suffix is present
            search_result = yf.download(
                tickers = f"^NSEI {symbol_search}.NS {symbol_search}.BO",
                period = "1d",
                progress = False
            )
            ...
            if not search_result.empty:
                for col in search_result.columns:
                    if not search_result[col].isna().all():
                        actual_symbol = col[0]
